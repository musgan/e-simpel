...
{
    protected static $listIndex = 100000;
    ...
    protected static function parseNode($node, $element, $styles = [], $data = []): void
    {
        ...
        $nodes = [
            ...
            'ul' => ['List',        $node,  $element,   $styles,    $data,  3,           null],
            'ol' => ['List',        $node,  $element,   $styles,    $data,  7,           null],
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(['App\Helpers\ErpHtml', $method], array_values($arguments));
    ...
    protected static function parseList($node, $element, &$styles, &$data)
    {
        ...
        if (isset($data['listdepth'])) {
            ...
            $data['listdepth'] = 0;
            $styles['list'] = 'style_default_'. self::$listIndex++;
//            $style = $element->getPhpWord()->addNumberingStyle($styles['list'], self::getListStyle($isOrderedList));
            $style = $element->getPhpWord()->addNumberingStyle($styles['list'], self::getListStyle(false));
