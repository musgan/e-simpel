...
<div class="card shadow mb-4">
    <div class="card-header py-3"  style="background-color: #{{$sector->base_color}}">
      <h6 class="m-0 font-weight-bold"  style="color: {{CostumHelper::getContrastColor('#'.$sector->base_color)}}">{{ucfirst($sub_menu)}}<i class="fas fa-fw fa-chevron-right"></i> Update</h6>
    </div>
    <div class="card-body">
    	@if ($errors->any())
	        <div class="row">
	    ...
	    <div class="action-btn btn-back">
	      	...
	      	{{
	      		((session('backlink_tl'.$sub_menu))) ? session('backlink_tl'.$sub_menu) :
	      		url(session('role').'/tindak-lanjutan/'.strtolower($sector->category).'/'.$sub_menu)
	    ...
	    {!! Form::model($send, [
	          'method' =>'PUT',
	          'url' => [session('role').'/tindak-lanjutan/'.strtolower($sector->category).'/'.$sub_menu, $send->id],
